  ...
  return (
    <Card className={`bg-casino-card-bg border-casino-border-subtle overflow-hidden transition-colors duration-200 hover:border-casino-neon-green focus:border-casino-neon-green active:border-casino-neon-green hover:${getSafetyGlow(casino.safetyIndex)} group cursor-pointer w-full h-full flex flex-col`}>
      <div className="p-3 md:p-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-y-2 mb-3 md:mb-4">
          <div className="flex items-center space-x-2 md:space-x-4">
            <div className="w-10 h-10 md:w-14 md:h-14 bg-white rounded-lg overflow-hidden flex items-center justify-center flex-shrink-0">
              <Image
                src={casino.logo}
                alt={casino.name}
                width={56}
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center flex-wrap gap-1 md:gap-2 mb-1">
                <h3 className="text-base md:text-xl font-bold text-white truncate">{casino.name}</h3>
                {casino.isNew && (
              ...
              <div className="flex items-center space-x-1 md:space-x-2">
                <div className="flex items-center">
                  {[...Array(5)].map((_, i) => {
                    ...
                    if (diff >= 1) {
                      // Full star
                      return <Star key={i} className="w-3 h-3 md:w-4 md:h-4 text-yellow-400 fill-current" />;
                    } else if (diff >= 0.5) {
                      // Half star
                      return <StarHalf key={i} className="w-3 h-3 md:w-4 md:h-4 text-yellow-400 fill-current" />;
                    }
                    // Empty star
                    return <Star key={i} className="w-3 h-3 md:w-4 md:h-4 text-gray-600" />;
                  })}
                </div>
                <span className="text-gray-400 text-xs md:text-sm">({casino.rating}/5)</span>
              </div>
            </div>
          </div>
        ...
        {/* Bonus Offer */}
        <div 
          className={`bg-gradient-to-r from-casino-neon-green/10 to-casino-neon-purple/10 border rounded-lg p-2 md:p-3 mb-3 md:mb-4 cursor-pointer transition-all duration-200 ${
            isCurrentCasino 
        ...
        >
          <div className="flex flex-wrap items-center justify-between gap-x-2 md:gap-x-4 gap-y-1 md:gap-y-2 mb-1 md:mb-2">
            <div className="flex items-center">
              <Gift className="w-3 h-3 md:w-4 md:h-4 mr-1 md:mr-2 text-casino-neon-green" />
              <span style={{ fontFamily: 'Space Grotesk, sans-serif' }} className="font-semibold text-xs md:text-sm text-casino-neon-green">{casino.bonus}</span>
            </div>
          </div>
        </div>
        ...
        {/* Description */}
        <p className="text-gray-300 text-xs md:text-sm mb-3 md:mb-4 line-clamp-2 leading-relaxed">{casino.description}</p>
        ...
        <div className="space-y-2 md:space-y-3">
          ...
          <a
            ...
            className="block"
          >
            <Button className="w-full bg-casino-neon-green text-casino-dark font-bold text-sm md:text-lg py-2 md:py-3 hover:bg-casino-neon-green/90 transition">
          ...
          <div className="grid grid-cols-2 gap-1 md:gap-2">
            ...
            >
              <Gift className="w-2.5 h-2.5 md:w-3 md:h-3 mr-0.5 md:mr-1" />
