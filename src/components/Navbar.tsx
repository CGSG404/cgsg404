...
import { Menu, X, Star, User, LogOut, UserCircle, Settings, LayoutDashboard, Dice5, Gamepad2, Gift, BookOpen, MessageSquare, Trophy, Newspaper, ChevronRight } from 'lucide-react';
...
const Navbar = () => {
  ...
  const navLinks = [
    { name: 'Casinos', href: '/casinos', icon: Dice5 },
    { name: 'Top Casinos', href: '/games', icon: Trophy },
    { name: 'Reviews', href: '/reviews', icon: Star },
    { name: 'List Report', href: '/list-report', icon: LayoutDashboard },
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.href}
                className="flex items-center gap-2 text-gray-300 hover:text-casino-neon-green transition-colors duration-200 font-medium"
              >
                {link.icon && <link.icon className="h-4 w-4 text-casino-neon-green" />}
                <span>{link.name}</span>
              </Link>
            ))}
          </div>
          ...
          {isMenuOpen && (
            <>
              ...
              <motion.aside
                ...
                className="md:hidden w-full py-4 border-t border-casino-border-subtle bg-casino-card-bg/95 backdrop-blur-md px-4"
              >
                <div className="space-y-6">
                  {user && (
                    <button
                      ...
                      className="w-full text-left text-gray-300 hover:text-casino-neon-green flex items-center gap-2 transition-colors"
                    >
                  ...
                  {navLinks.map((link) => (
                    <Link
                      key={link.name}
                      href={link.href}
                      className="flex items-center justify-between gap-2 text-gray-300 hover:text-casino-neon-green text-base font-medium py-2 transition-colors"
                  ...
                  {user ? (
                    <button
                      ...
                      className="w-full text-left text-red-500 hover:text-red-400 flex items-center gap-2 transition-colors"
                    >
